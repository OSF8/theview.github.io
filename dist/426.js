(self["webpackChunkknight_frank_the_view"] = self["webpackChunkknight_frank_the_view"] || []).push([[426],{

/***/ "./node_modules/@deleteagency/device-observer/src/index.ts":
/*!*****************************************************************!*\
  !*** ./node_modules/@deleteagency/device-observer/src/index.ts ***!
  \*****************************************************************/
...
class DeviceObserver {
  constructor(devices, options) {
    ...
    this._sortDevices = devices => Object.keys(devices).sort((a, b) => devices[a] - devices[b]).map((name, index) => ({
      size: devices[name],
      name,
      index
    }));
    ...
    this.isGreaterOrEqual = pixelsValue => matchMedia("only screen and (min-width:".concat(pixelsValue, "px)")).matches;
    /**
     * @param {number} pixelsValue - width in pixels to compare with
     * @return {boolean}
     */
    ...
    this.isLowerOrEqual = pixelsValue => matchMedia("only screen and (max-width:".concat(pixelsValue === Infinity ? 10000 : pixelsValue, "px)")).matches;

    this._getDeviceIndex = deviceName => this._devices.findIndex(device => device.name === deviceName);

    this._devices = this._sortDevices(devices);
    if (options) this._options = _extends(this._options, options);
    const device = this._getCurrentDevice;
    if (device) this._currentDevice = device;
    window.addEventListener('resize', this._onWindowResize);
  }

  get _getCurrentDevice() {
    return this._options.mobileFirst ? [...this._devices].reverse().find(_ref => {
      let {
        size
      } = _ref;
      return this.isGreaterOrEqual(size);
    }) : this._devices.find(_ref2 => {
      let {
        size
      } = _ref2;
      return this.isLowerOrEqual(size);
    });
  }
  ...
  subscribeOnChange(cb) {
    this._deviceChangeCallbacks.push(cb); // return unsubscribe method
